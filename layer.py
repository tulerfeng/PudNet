...
class CNR2d(nn.Module): 
    def __init__(self, nch_in, nch_out, kernel_size=4, stride=1, padding=1, norm='bnorm', relu=0.0, drop=[], bias=[]):
        ...
        self.conv = Conv2d(bias=bias)
        self.bn = Norm2d(nch_out, norm)
        self.relu = ReLU(relu)

    def forward(self, x, conv_w):
        fea = self.conv(x, conv_w)
        fea = self.bn(fea)
        fea = self.relu(fea)
        return fea
...
class DECNR2d(nn.Module): 
    def __init__(self, nch_in, nch_out, kernel_size=4, stride=1, padding=1, output_padding=0, norm='bnorm', relu=0.0, drop=[], bias=[]):
        ...
        self.deconv = Deconv2d(padding=padding, output_padding=output_padding, bias=bias)
        self.bn = Norm2d(nch_out, norm)
        self.relu = ReLU(relu)

    def forward(self, x, conv_w):
        fea = self.deconv(x, conv_w)
        fea = self.bn(fea)
        fea = self.relu(fea)
        return fea
...
class Conv2d(nn.Module):
    ...
    def forward(self, x, conv_w):
        fea = F.conv2d(x, conv_w, bias=None, stride=1, padding=1)
        return fea


class Deconv2d(nn.Module):
    def __init__(self, kernel_size=3, stride=1, padding=1, output_padding=0, bias=True):
        super(Deconv2d, self).__init__()
        

    def forward(self, x, deconv_w):
        # return self.deconv(x)
        fea = F.conv_transpose2d(x, deconv_w, bias=None, stride=1, padding=1)
        return fea
